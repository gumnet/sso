...
class Response
{
    ...
    public function __construct(Settings $settings, $response)
    {
        ...
        $this->document = (new Utils())->loadXML($this->document, $this->response);
        if (!$this->document) {
        ...
        try {
            ...
            if ($this->settings->isStrict()) {
                ...
                if ($security['wantXMLValidation']) {
                    ...
                    $res = (new Utils())->validateXML($this->document, 'saml-schema-protocol-2.0.xsd', $this->settings->isDebugActive(), $this->settings->getSchemasPath());
                    ...
                    if ($this->encrypted) {
                        $res = (new Utils())->validateXML($this->decryptedDocument, 'saml-schema-protocol-2.0.xsd', $this->settings->isDebugActive(), $this->settings->getSchemasPath());
                        if (!$res instanceof \DOMDocument) {
                            throw new ValidationError(
                                $errorXmlMsg,
                                ValidationError::INVALID_XML_FORMAT
                ...
                $currentURL = (empty($_SERVER['HTTPS']) ? 'http' : 'https')
                ...
                if ($this->document->documentElement->hasAttribute('Destination')) {
                    ...
                    if (empty($destination)) {
                        ...
                        $urlComparisonLength =
                            $security['destinationStrictlyMatches'] ? strlen($destination) : strlen($currentURL);
                ...
                foreach ($subjectConfirmationNodes as $scn) {
                    ...
                    if ($subjectConfirmationDataNodes->length == 0) {
                        ...
                        if ($scnData->hasAttribute('NotOnOrAfter')) {
                            $noa = (new Utils())->parseSAML2Time($scnData->getAttribute('NotOnOrAfter'));
                            if ($noa + Constants::ALLOWED_CLOCK_DRIFT <= time()) {
                                continue;
                            }
                        }
                        if ($scnData->hasAttribute('NotBefore')) {
                            $nb = (new Utils())->parseSAML2Time($scnData->getAttribute('NotBefore'));
            ...
            if ($this->encrypted) {
                $encryptedIDNodes = (new Utils())->query(
            ...
            if (empty($signedElements) || (!$hasSignedResponse && !$hasSignedAssertion)) {
                ...
                $multiCerts = null;
                $existsMultiX509Sign = isset($idpData['x509certMulti']) && isset($idpData['x509certMulti']['signing'])
                    && !empty($idpData['x509certMulti']['signing']);
                ...
                if ($hasSignedResponse && ! (new Utils())->validateSign(
                    $this->document,
                    $cert,
                    $fingerprint,
                    $fingerprintalg,
                    Utils::RESPONSE_SIGNATURE_XPATH,
                    $multiCerts
                    )) {
                    throw new ValidationError(
                ...
                if ($hasSignedAssertion && ! (new Utils())->validateSign(
    ...
    {
        $status = (new Utils)->getStatus($this->document);
    ...
    */
    ...
    {
        $entries = $this->queryAssertion("/saml:Conditions");
        if ($entries->length == 1) {
            return true;
        } else {
            return false;
    ...
    */
    ...
    {
        $entries = $this->queryAssertion("/saml:AuthnStatement");
        if ($entries->length == 1) {
            return true;
        } else {
            return false;
    ...
    public function getIssuers()
    {
        ...
        $responseIssuer = (new Utils())->query($this->document, '/samlp:Response/saml:Issuer');
    ...
    public function getSessionNotOnOrAfter()
    {
        ...
        if ($entries->length !== 0) {
            $notOnOrAfter = (new Utils())->parseSAML2Time($entries->item(0)->getAttribute('SessionNotOnOrAfter'));
        }
        return $notOnOrAfter;
    }
     ...
     * @throws ValidationError
     */
    private function getAttributesByKeyName($keyName = "Name")
    {
        $attributes = array();
    ...
    public function validateTimestamps()
    {
        ...
        for ($i = 0; $i < $timestampNodes->length; $i++) {
            ...
            if ($nbAttribute && (new Utils())->parseSAML2Time($nbAttribute->textContent) > time() + Constants::ALLOWED_CLOCK_DRIFT) {
                throw new ValidationError(
                    'Could not validate timestamp: not yet valid. Check system clock.',
                    ValidationError::ASSERTION_TOO_EARLY
                );
            }
            if ($naAttribute && (new Utils())->parseSAML2Time($naAttribute->textContent) + Constants::ALLOWED_CLOCK_DRIFT <= time()) {
                throw new ValidationError(
    ...
    public function validateSignedElements($signedElements)
    {
        ...
        if (in_array($responseTag, $signedElements)) {
            $expectedSignatureNodes = (new Utils())->query($this->document, Utils::RESPONSE_SIGNATURE_XPATH);
    ...
    {
        if ($this->encrypted) {
            return (new Utils())->query($this->decryptedDocument, $query);
        } else {
            return (new Utils())->query($this->document, $query);
        }
    }
